...
void CFUPS::proc_(QByteArray data) { // 该函数只能被CFUPSManager调用
    if (data.size() <= IV_LEN)return; // 长度不够
    ...
    if (data.size() < 17)return; // 长度不够
    auto rand = data.mid(0, HEAD_RAND_LEN);
    if (peerRandSet.contains(rand))return; // 确保是没接收过的
    peerRandSet.insert(rand);

    data = data.mid(HEAD_RAND_LEN); // 撇去16个随机数
    ...
    if (1 <= cmd && cmd <= 5 && !UDL) {
        ...
        if (!NA && UD) {//需要回复
            ...
            if (!initiative && cs == 1 && ID == 2 && OID == 1 && SID == 2 && data.mid(11).size() == IV_LEN && !UDL) {
                IV = data.mid(11);
                OID++;
                NA_ACK_(SID);
            } else {
...
void CFUPS::sendPackage_(CDPT *cdpt) { // 只负责构造数据包和发送
    ...
    } while (localRandSet.contains(rand)); // 生成唯一的
    localRandSet.insert(rand);
    data = rand + data;
...
void CFUPS::NA_ACK_(unsigned short AID) {
    auto cdpt = new CDPT(this);
    cdpt->AID = AID;
    cdpt->cf = 0x22;
    sendPackage_(cdpt);
...
bool CFUPS::time_(unsigned short SID, long long time) {
